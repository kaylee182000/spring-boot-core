...
public interface RoleRepository extends JpaRepository<Role, Long> {
        ...
        @Query(value = "SELECT p.*"
                        +
                        "FROM roles r left join roles_permissions rp on r.id=rp.roleId left join permissions p on p.id=rp.permissionId  "
                        +
                        "WHERE r.id= :roleId", nativeQuery = true)
        Set<Permission> findPermissionByRoleId(@Param("roleId") Long roleId);
